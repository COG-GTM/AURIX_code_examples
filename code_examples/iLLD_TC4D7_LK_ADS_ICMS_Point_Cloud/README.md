...
# iLLD_TC4D7_LK_ADS_ICMS_Point_Cloud
**This software project for the TC4D7 Lite Kit (AURIX™ TC4D7 and BGT60 ATR24AIP) demonstrates the main radar signal processing steps to generate point cloud data from raw data recorded by the MMIC.**  


## Device  
The device used in this example is AURIX™ TC4D7XP_A-Step_MC_COM.
...
This software project is intended as a starting point for the application of in cabin monitoring (ICMS). It demonstrates the main processing steps that need to be performed on AURIX™ TC4D7 signal processing unit (SPU) and Tricore CPUs to generate point cloud data from raw data recorded by the BGT60 ATR24AIP MMIC. 

Basic understanding of FMCW radar range-doppler signal processing is presumed to follow this code example.

This project is a TC4xx migration of the original TC3xx example `iLLD_TC356TA_ADS_ICMS_BOARD_3_0_point_cloud`.


## Introduction  
AURIX™ radar microcontrollers use a hardware accelerator called the Signal Processing Unit (SPU) to extract information from radar data. 
...
The radar data cube and both intermediate and final results are stored in the AURIX™ TC4D7 Extension memory (EMEM). This is a dedicated memory for radar data.
...
## Hardware setup  
This code example was developed for the AURIX™ TC4D7 Lite Kit (KIT_A3G_TC4D7_LITE).
...
The main objective of this code is to establish SPI communication between the AURIX™ TC4D7 microcontroller and the BGT60ATR24EAIP, enable the BGT60's 40MHz clock output and switch the TC4D7 clock to this external source. After switching to the external clock source, the SPI module needs to be re-initialized, and then also the STM, DMA and RMEM modules are initialized. Upon successful initialization of these hardware modules, CPU0 enters the RADAR State Machine which handles the main RADAR application flow.
