...
# iLLD_TC4D7_LK_ADS_EGTM_TOM_3_Phase_Inverter_PWM_2 
 
**The EGTM TOM is configured to generate PWM signals for two-level three phase inverter. The TriCore&trade; is used as a host for the driver.**  
...
Module (EGTM) in-built Timer Output Module (TOM). All signals are synchronous to each
...
The Timer Output Module (TOM), which is part of the EGTM, can offer up to 16 independent channels to generate output signals.

The Clock Management Unit (CMU) is responsible for clock generation of the EGTM. The Configurable Clock Generation Subunit (CFGU) provides eight clock sources for the EGTM submodules: TIM, TBU, MON and TOM.
...
The *initEgtmTomPwm()* configuration sequence is the following:
* Initialization of the EGTM Module
* Configuration of the CMU-CLOCK0 for the TOM usage
...
<table>
    <tbody>
        ...
        <tr>
            ...
            <td>&emsp;<span style="color:blue">P00.4</span></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>&emsp;<span style="color:green">P00.6</span></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>&emsp;<span style="color:red">P00.8</span></td>
        </tr>
    </tbody>
</table>
...
Once the EGTM is configured and started, a duty cycle update is performed every 10ms in the *updateEgtmTomPwmDutyCycles()* function:

1. Each channel **x** is cyclically modified with 10% increment duty cycle from 10% till 90% using the variable *g_egtmTom3phInv.dutyCycles[***x***]* 
