/**********************************************************************************************************************
 * \file EGTM_ATOM_CLLC_Converter_PWM.c
...
#define HVHV_PRI_PWM_T1      &IfxEgtm_ATOM0_0_TOUT9_P00_0_OUT        /* Pin which will be driven by the PWM, P00.0  */
#define HVHV_PRI_PWM_T2      &IfxEgtm_ATOM0_0N_TOUT17_P00_8_OUT      /* Pin which will be driven by the PWM, P00.8  */
#define HVHV_PRI_PWM_T3      &IfxEgtm_ATOM0_1_TOUT11_P00_2_OUT       /* Pin which will be driven by the PWM, P00.2  */
#define HVHV_PRI_PWM_T4      &IfxEgtm_ATOM0_1_TOUT10_P00_1_OUT       /* Pin which will be driven by the PWM, P00.1  */
/* Low-voltage side (Secondary) */
#define HVHV_SEC_PWM_T1      &IfxEgtm_ATOM0_2_TOUT33_P33_11_OUT      /* Pin which will be driven by the PWM, P33.11 */
#define HVHV_SEC_PWM_T2      &IfxEgtm_ATOM0_2N_TOUT23_P33_1_OUT      /* Pin which will be driven by the PWM, P33.1  */
#define HVHV_SEC_PWM_T3      &IfxEgtm_ATOM0_3_TOUT49_P22_2_OUT       /* Pin which will be driven by the PWM, P22.2  */
#define HVHV_SEC_PWM_T4      &IfxEgtm_ATOM0_3N_TOUT42_P23_1_OUT      /* Pin which will be driven by the PWM, P23.1  */
...
IFX_STATIC EgtmAtomCLLC g_egtmAtomCLLC;

/*********************************************************************************************************************/
/*-----------------------------------------------Function Prototypes-------------------------------------------------*/
/*********************************************************************************************************************/
static void dutyEventFunction(void *data);             /* Callback function for ATOM CHO for CCU1 Event              */
void updateEgtmAtomCLLCPhaseAndFrequency(void);        /* Function to set the phase and frequency of the PWM signals */
...
{
    updateEgtmAtomCLLCPhaseAndFrequency();
}
...
void initEgtmAtomCLLC(void)
{
    ...
    g_egtmAtomCLLC.primaryDisplacementAngle   = 0.0f;
    g_egtmAtomCLLC.secondaryDisplacementAngle = 0.0f;
    g_egtmAtomCLLC.frequency = PWM_FREQUENCY_DEFAULT;
    ...
    /* CH0 configurations */
    output[0].pin                   = (IfxEgtm_Pwm_ToutMap*)HVHV_PRI_PWM_T1;     /* HVLV Primary PWM High-side (T1)   */
    output[0].complementaryPin      = (IfxEgtm_Pwm_ToutMap*)HVHV_PRI_PWM_T2;     /* HVLV Primary PWM Low-side (T2)    */
    ...
    /* CH1 configurations */
    output[1].pin                   = (IfxEgtm_Pwm_ToutMap*)HVHV_PRI_PWM_T3;     /* HVLV Primary PWM High-side (T3)   */
    output[1].complementaryPin      = (IfxEgtm_Pwm_ToutMap*)HVHV_PRI_PWM_T4;     /* HVLV Primary PWM Low-side (T4)    */
    ...
    /* CH2 configurations */
    output[2].pin                   = (IfxEgtm_Pwm_ToutMap*)HVHV_SEC_PWM_T1;     /* HVLV Secondary PWM High-side (T1) */
    output[2].complementaryPin      = (IfxEgtm_Pwm_ToutMap*)HVHV_SEC_PWM_T2;     /* HVLV Secondary PWM Low-side (T2)  */
    ...
    /* CH3 configurations */
    output[3].pin                   = (IfxEgtm_Pwm_ToutMap*)HVHV_SEC_PWM_T3;     /* HVLV Secondary PWM High-side (T3) */
    output[3].complementaryPin      = (IfxEgtm_Pwm_ToutMap*)HVHV_SEC_PWM_T4;     /* HVLV Secondary PWM Low-side (T4)  */
    ...
    config.frequency              = g_egtmAtomCLLC.frequency;            /* PWM frequency                              */
    ...
    IfxEgtm_Pwm_init(&g_egtmAtomCLLC.pwm, &g_egtmAtomCLLC.channels[0], &config);

    /* 10. Store the current duty values, dead-times and phases for runtime calls */
    g_egtmAtomCLLC.dutyCycles[0] = channelConfig[0].duty;
    g_egtmAtomCLLC.dutyCycles[1] = channelConfig[1].duty;
    g_egtmAtomCLLC.dutyCycles[2] = channelConfig[2].duty;
    g_egtmAtomCLLC.dutyCycles[3] = channelConfig[3].duty;
    g_egtmAtomCLLC.deadTimes[0] = channelConfig[0].dtm->deadTime;
    g_egtmAtomCLLC.deadTimes[1] = channelConfig[1].dtm->deadTime;
    g_egtmAtomCLLC.deadTimes[2] = channelConfig[2].dtm->deadTime;
    g_egtmAtomCLLC.deadTimes[3] = channelConfig[3].dtm->deadTime;
    g_egtmAtomCLLC.phases[0] = channelConfig[0].phase;
    g_egtmAtomCLLC.phases[1] = channelConfig[1].phase;
    g_egtmAtomCLLC.phases[2] = channelConfig[2].phase;
    g_egtmAtomCLLC.phases[3] = channelConfig[3].phase;

    g_egtmAtomCLLC.rtState = Paused;
}

/* Function to set the phase and frequency of the PWM signals */
void updateEgtmAtomCLLCPhaseAndFrequency(void)
{
    if(g_egtmAtomCLLC.rtState == Run)
    {
        /* Check the validity of displacement ratios */
        if(g_egtmAtomCLLC.primaryDisplacementAngle < 0.0f)
        {
            g_egtmAtomCLLC.primaryDisplacementAngle = 0.0f;
        }

        if(g_egtmAtomCLLC.primaryDisplacementAngle > IFX_PI)
        {
            g_egtmAtomCLLC.primaryDisplacementAngle = IFX_PI;
        }

        if(g_egtmAtomCLLC.secondaryDisplacementAngle < 0.0f)
        {
            g_egtmAtomCLLC.secondaryDisplacementAngle = 0.0f;
        }

        if(g_egtmAtomCLLC.secondaryDisplacementAngle > IFX_PI)
        {
            g_egtmAtomCLLC.secondaryDisplacementAngle = IFX_PI;
        }

        /* Check the validity of the resonant frequency */
        if(g_egtmAtomCLLC.frequency < PWM_FREQUENCY_MIN)
        {
            g_egtmAtomCLLC.frequency = PWM_FREQUENCY_MIN;
        }

        if(g_egtmAtomCLLC.frequency > PWM_FREQUENCY_MAX)
        {
            g_egtmAtomCLLC.frequency = PWM_FREQUENCY_MAX;
        }

        /* Calculate shift phases */
        /* Phase of HVLV_PRI_PWM_T1, HVLV_PRI_PWM_T2 */
        g_egtmAtomCLLC.phases[0] = 0.0f;
        /* Phase of HVLV_PRI_PWM_T3, HVLV_PRI_PWM_T4 */
        g_egtmAtomCLLC.phases[1] = g_egtmAtomCLLC.primaryDisplacementAngle + IFX_PI;
        /* Phase of HVLV_SEC_PWM_T1, HVLV_SEC_PWM_T2 */
        g_egtmAtomCLLC.phases[2] = g_egtmAtomCLLC.secondaryDisplacementAngle;
        /* Phase of HVLV_SEC_PWM_T3, HVLV_SEC_PWM_T4 */
        g_egtmAtomCLLC.phases[3] = g_egtmAtomCLLC.secondaryDisplacementAngle + IFX_PI;

        /* Update the iLLD Framework regarding frequency and phase shift */
        IfxEgtm_Pwm_updateChannelsPhase(&g_egtmAtomCLLC.pwm, (float32*)&g_egtmAtomCLLC.phases[0]);
        IfxEgtm_Pwm_updateFrequency(&g_egtmAtomCLLC.pwm, g_egtmAtomCLLC.frequency);
    }
}
