...
#define LED_OK              LED_1                       /* LED1, used to signal no errors */
#define LED_ERROR           LED_2                       /* LED2, used to signal an error */
...
static Flash_ResultType checkPFlashAddress(uint32 address, uint32 *rcode, IfxFlash_FlashType* type)
{
    ...
    if (result == NO_ERROR)
    {
        ...
        IfxWtu_clearCpuEndinitInline(&MODULE_WTU.WDTCPU[0], endInitCpuPassword);
        ...
        IfxWtu_setCpuEndinitInline(&MODULE_WTU.WDTCPU[0], endInitCpuPassword);
...
static Flash_ResultType checkDFlashAddress(uint32 address, uint32 *rcode)
{
    ...
    /* Get the current password of the CPU WatchDog module */
    endInitCpuPassword = IfxWtu_getCpuWatchdogPassword();
...
void initLEDs(void)
{
    ...
    IfxPort_setPinModeOutput(LED_OK.port, LED_OK.pinIndex, IfxPort_OutputMode_pushPull, IfxPort_OutputIdx_general);
    IfxPort_setPinModeOutput(LED_ERROR.port, LED_ERROR.pinIndex, IfxPort_OutputMode_pushPull, IfxPort_OutputIdx_general);

    /* Switch OFF the LED (low-level active) */
    IfxPort_setPinHigh(LED_OK.port, LED_OK.pinIndex);
    IfxPort_setPinHigh(LED_ERROR.port, LED_ERROR.pinIndex);
}
...
void blinkLED(Flash_ResultType error_type)
{
    switch (error_type)
    {
        ...
        case NO_ERROR:
        {
            IfxPort_togglePin(LED_OK.port, LED_OK.pinIndex);
        ...
        default:
        {
            IfxPort_togglePin(LED_ERROR.port, LED_ERROR.pinIndex);
            waitTime(IfxStm_getTicksFromMilliseconds(WAIT_TIME));
            break;
        }
    }
...
{
    /* Switch on the LED2 */
    IfxPort_setPinLow(LED_ERROR.port, LED_ERROR.pinIndex);
