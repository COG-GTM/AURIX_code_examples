 ...
 * \abstract This software project for the TC4D7 lite kit demonstrates the main radar signal processing steps to generate point cloud data from raw data recorded by the MMIC and to calculate the breathing rate of a detected target.
 * \description This software project is intended as a starting point for the application of in cabin monitoring (ICMS) on the TC4D7 lite kit. It demonstrates the main processing steps that need to be performed on AURIX TC4D7 signal processing unit (SPU) and Tricore CPUs to generate point cloud data from raw data recorded by the BGT60 ATR24AIP MMIC and to calculate the breathing rate of a detected target.
 *              
 *              Basic understanding of FMCW radar range-doppler signal processing is presumed to follow this code example.
 *
 * \name iLLD_TC4D7_LK_ADS_ICMS_Breathing_Rate
 * \version V1.0.0
