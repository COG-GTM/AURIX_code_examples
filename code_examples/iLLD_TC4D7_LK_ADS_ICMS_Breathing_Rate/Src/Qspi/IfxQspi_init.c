...
void Qspi_Init(float32 f_qspi)
{
    ...
    uint16 passwd = IfxWtu_getCpuWatchdogPassword();
    IfxWtu_clearSafetyEndinitInline(passwd);
    scu->CCUCON1.B.CLKSELQSPI = 1; /*fsource1 is used as clock source fSOURCEMSC*/
    scu->CCUCON1.B.QSPIDIV    = 1; /*fQSPI = fSOURCEQSPI*/
    IfxWtu_setSafetyEndinitInline(passwd);
    IfxClock_setQspiFrequency(f_qspi);
...
void Qspi_Mode(uint32 mode)
{
    ...
    if (mode==Spi_Single)
    {
        ...
        IfxSrc_init(src, ISR_PROVIDER_CPU0, ISR_PRIORITY_MMIC_SPI_RX, IfxSrc_VmId_0); /*Serve RX with CPU0*/
        IfxSrc_enable(src);
        /* SPI transmit interrupt CPU*/
        src = IfxQspi_getSrcPointerIntTx(spi->spiSfr);
        IfxSrc_init(src, ISR_PROVIDER_CPU0, ISR_PRIORITY_MMIC_SPI_TX, IfxSrc_VmId_0); /*Serve TX with CPU0*/
        IfxSrc_enable(src);
    }
    else
    {
        ...
        src = IfxQspi_getSrcPointerIntRx(spi->spiSfr);
        IfxSrc_init(src, ISR_PROVIDER_DMA, ISR_DMA_CH_MMIC_SPI_RX, IfxSrc_VmId_0); /* Serve RX with DMA Channel 1*/
        IfxSrc_enable(src);
        /* SPI transmit interrupt DMA*/
        src = IfxQspi_getSrcPointerIntTx(spi->spiSfr);
        IfxSrc_init(src, ISR_PROVIDER_DMA, ISR_DMA_CH_MMIC_SPI_TX, IfxSrc_VmId_0); /* Serve TX with DMA Channel 0*/
        IfxSrc_enable(src);
    }
}
