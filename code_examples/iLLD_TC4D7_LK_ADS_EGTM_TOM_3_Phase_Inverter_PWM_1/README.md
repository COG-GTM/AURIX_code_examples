...
# iLLD_TC4D7_LK_ADS_EGTM_TOM_3_Phase_Inverter_PWM_1 
 
**The EGTM TOM is configured to generate PWM signals for a two-level three phase inverter. The TriCore&trade; is used as a host for the driver.**  
...
Module (EGTM) Timer Output Module (TOM). All signals are synchronous to each
...
The Timer Output Module (TOM), which is part of the EGTM, is able to generate complex output signals including center-aligned PWM with complementary outputs and dead-time insertion via the DTM (Dead Time Module).

The Clock Management Unit (CMU) is responsible for clock generation of the EGTM. The Fixed Clock Generation Subunit (FXCLK) provides clock sources for the TOM submodule.
...
The *initEgtmTom3phInv()* configuration sequence is the following:
* Initialization of the EGTM Module
* Configuration of the CMU-FXCLK0 for the TOM usage and CMU-CLK0 for the DTM usage
...
1. Each channel **x** is cyclically modified with 10% increment duty cycle from 0% till 100% using the variable *g_egtmTom3phInv.dutyCycles[***x***]* 
2. The duty cycle of all channels is then updated at once using the iLLD function *IfxEgtm_Pwm_updateChannelsDutyImmediate()*
3. To indicate the correct functioning of the application, the LED (P03.9) is toggled once in a second using the iLLD function *IfxPort_togglePin()*
