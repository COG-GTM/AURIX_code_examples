...
# iLLD_TC4D7_LK_ADS_BUS_Memory_Protection
**CPU0 and CPU1 handle read/write access permission to the local memory of CPU0 from the bus master CPU1.**
...
bus master CPU1 does not have access permission to that area. Different test scenarios have been provided to
define access permission for each local memory.
...
DSPR, DLMU, SRAM) of each core. In this code example, the BUS MPU module is focused on CPU0 and CPU1,
handling "read/write" access permission to the local memory of CPU0 (DSPR0/PSPR0/DLMU0/PFlash0) from the bus master CPU1. The BUS
...
**Note:** In TC4xx, the BUS MPU access enable registers use a different naming convention compared to TC3xx.
The registers are named CPUx_ACCENSPRi and CPUx_ACCENDLMUi (instead of CPUx_SPR_SPROT and CPUx_DLMU_SPROT).
Additionally, TC4xx does not have LPB_SPROT registers; PFlash protection is handled differently.

**Note:** In TC4xx, the ACCEN registers use resource protection (PROTSPRSE/PROTDLMUSE) instead of safety endinit
...
<br><br>
- The registers, *CPUx_ACCENSPRi_RGNLA* (i=0-7) to define the lower address of the region, *CPUx_ACCENSPRi_RGNUA*
...
<br><br>
- The registers, *CPUx_ACCENDLMUi_RGNLA* (i=0-7) to define the Lower address of the region, *CPUx_ACCENDLMUi_RGNUA*
...
(TAG ID based protection), that indicates which masters are allowed to reach and access the region.
<br><br>
...
number, the master TAG number identifying up to 64 masters (*see the On Chip Bus chapter of User Manual, AURIX&trade; TC4xx*).
<br>
- *Access enable register:*

Access enable register specifies which of the possible masters are allowed to perform access and which ones are blocked. This register controls access for transactions with the on chip bus master TAG ID, and provides one enable bit for
each possible 6-bit TAG ID.
...
*CPUx_ACCENSPRi_RDA/WRA* (i=0-7) (Masters 31-0) and *CPUx_ACCENSPRi_RDB/WRB* (i=0-7) (Masters 63-32).
<br>
- A read/write access to the PSPR/DSPR memory is seen as valid if the master tag of the access is enabled in the
*CPUx_ACCENSPRi_RDA/WRA*  register and the address of the access satisfies the following relationship:
*CPUx_ACCENSPRi_RGNLA <= address < CPUx_ACCENSPRi_RGNUA*.
...
*CPUx_ACCENDLMUi_RDA/WRA* (i=0-7) (Masters 31-0) and *CPUx_ACCENDLMUi_RDB/WRB* (i=0-7) (Masters 63-32).
<br><br>
- A read/write access to the DLMU memory is seen as valid if the master tag of the access is enabled in the
*CPUx_ACCENDLMUi_RDA/WRA* register and the address of the access satisfies the following relationship:
*CPUx_ACCENDLMUi_RGNLA <= address < CPUx_ACCENDLMUi_RGNUA*
- If any of these conditions are not satisfied, the access is seen as invalid.
...
<br>
- **Note:** TC4xx does not have LPB_SPROT registers. PFlash protection is handled differently in TC4xx.
The LPB enable/disable functions are kept as stubs for API compatibility.
...
*disable_DLMU0_write_bus_protection_range()*: &nbsp;&nbsp;&nbsp; disable DLMU0 write access to the region <br>
<br>
*enable_LPB0_read_bus_protection_range()*: &nbsp;&nbsp;&nbsp;&nbsp; enable LPB0 read access (stub on TC4xx) <br>
*disable_LPB0_read_bus_protection_range()*: &nbsp;&nbsp;&nbsp;&nbsp;disable LPB0 read access (stub on TC4xx) <br>
<br>
...
**Note:** Core0 and Core1 are *"SYNCHRONIZED"*, Core1 is waiting for Core0 configuration to do execute, and Core0 is waiting for Core1 execute to do configuration.
<br>
...
- write access: Silently ignored, and No trap is triggered actually.
<br><br>
- read access : DSE trap is triggered, (Data Access Synchronous Error, Class = 4, TIN=2).<br>
ALMx [1] alarm is triggered and *LBE* bit of *DSTR* register is set. *LBE* bit set leads to DSE trap (*see On-chip Bus chapter under SRI errors in the User Manual, AURIX&trade; TC4xx*).
...
No Trap triggered (LED_SUCCESS): ON &rarr; P03.10 (LED2) : ON

Trap triggered (LED_ERROR): ON &rarr; P03.9 (LED1) : ON
