...
# iLLD_TC4D7_LK_ADS_LED_DoubleClick_Button  
...
When the timer has expired, the state of the corresponding LED will change according to the number of times the button has been pressed.  
...
- The LEDs port pin state is set on or off with the functions *IfxPort_setPinState()* from the iLLD *IfxPort.h*  
...
- While the button is pressed once, LED1 changes state  
- While the button is pressed twice, LED2 changes state  
