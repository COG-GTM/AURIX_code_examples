...
#define PWM_OUT                  &IfxEgtm_TOM1_4_TOUT14_P00_5_OUT      /* LED which is driven by the PWM, P00.5      */
...
/*-------------------------------------------------Global variables--------------------------------------------------*/
/*********************************************************************************************************************/
IFX_STATIC EgtmTomPwm g_egtmTomPwm;                              /* Structure for PWM generation                      */
...
void initEgtmTomPwm(void)
{
    ...
    channelConfig[0].timerCh      = IfxEgtm_Pwm_SubModule_Ch_4;          /* Tom channel index                         */
    ...
    pwmConfig.cluster                = IfxEgtm_Cluster_1;                /* Cluster                                   */
    pwmConfig.subModule              = IfxEgtm_Pwm_SubModule_tom;        /* Sub module                                */
    ...
    pwmConfig.clockSource.tom        = IfxEgtm_Cmu_Fxclk_0;             /* Clock source for Tom                      */
    ...
    IfxEgtm_Pwm_init(&g_egtmTomPwm.pwm, &g_egtmTomPwm.channels[0], &pwmConfig);

    /* Store the current duty value for runtime calls */
    g_egtmTomPwm.dutyCycles[0] = channelConfig[0].duty;
}
...
{
    g_egtmTomPwm.dutyCycles[0] = dutyCycle;                          /* Set PWM duty cycle                            */

    /* Update duty of all configured channels to requested values immediately */
    IfxEgtm_Pwm_updateChannelsDutyImmediate(&g_egtmTomPwm.pwm, (float32*)g_egtmTomPwm.dutyCycles);
}
