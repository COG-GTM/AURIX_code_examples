/**********************************************************************************************************************
 * \file EGTM_ATOM_Dual_Active_Bridge_PWM.c
...
/*-----------------------------------------------------Includes------------------------------------------------------*/
/*********************************************************************************************************************/
#include "EGTM_ATOM_Dual_Active_Bridge_PWM.h"
#include "Ifx_Types.h"
...
/* Master Channel */
#define MASTER_CHANNEL       &IfxEgtm_ATOM2_0_TOUT32_P33_10_OUT       /* Pin which will be driven by the PWM, P33.10 */
/* High-voltage side(Primary) */
#define HVLV_PRI_PWM_T1      &IfxEgtm_ATOM2_1_TOUT27_P33_5_OUT        /* Pin which will be driven by the PWM, P33.5  */
#define HVLV_PRI_PWM_T2      &IfxEgtm_ATOM2_1N_TOUT34_P33_12_OUT      /* Pin which will be driven by the PWM, P33.12 */
#define HVLV_PRI_PWM_T3      &IfxEgtm_ATOM2_2_TOUT28_P33_6_OUT        /* Pin which will be driven by the PWM, P33.6  */
#define HVLV_PRI_PWM_T4      &IfxEgtm_ATOM2_2N_TOUT35_P33_13_OUT      /* Pin which will be driven by the PWM, P33.13 */
/* Low-voltage side(Secondary) */
#define HVLV_SEC_PWM_T1      &IfxEgtm_ATOM2_3_TOUT21_P00_12_OUT       /* Pin which will be driven by the PWM, P00.12 */
#define HVLV_SEC_PWM_T2      &IfxEgtm_ATOM2_3N_TOUT33_P33_11_OUT      /* Pin which will be driven by the PWM, P33.11 */
#define HVLV_SEC_PWM_T3      &IfxEgtm_ATOM2_4_TOUT30_P33_8_OUT        /* Pin which will be driven by the PWM, P33.8  */
#define HVLV_SEC_PWM_T4      &IfxEgtm_ATOM2_4N_TOUT29_P33_7_OUT       /* Pin which will be driven by the PWM, P33.7  */
...
typedef struct
{
    ...
    IfxEgtm_Pwm_DeadTime     deadTimes[NUM_OF_CHANNELS];     /* DeadTime values to hold                               */
...
IFX_STATIC EgtmAtomDAB g_egtmAtomDAB;

/*********************************************************************************************************************/
/*-----------------------------------------------Function Prototypes-------------------------------------------------*/
/*********************************************************************************************************************/
static void dutyEventFunction(void *data);                  /* Callback function for ATOM CH0 for CCU1 Event         */
void updateEgtmAtomDABPhase(float32 spsRadIn);              /* Function to set the duty cycle of the PWM             */
...
/* Callback function for ATOM CH0 for CCU1 Event */
void dutyEventFunction(void *data)
{
    updateEgtmAtomDABPhase(g_egtmAtomDAB.spsRad);
}

/* This function initializes the ATOM */
void initEgtmAtomDAB(void)
{
    IfxEgtm_Pwm_Config config;                                 /* Main PWM configuration structure                    */
    IfxEgtm_Pwm_ChannelConfig channelConfig[NUM_OF_CHANNELS];  /* Array containing configuration for PWM Channels     */
    IfxEgtm_Pwm_DtmConfig dtmConfig[NUM_OF_CHANNELS];          /* Array Containing Dead Time Values for PWM Channels  */
    IfxEgtm_Pwm_OutputConfig output[NUM_OF_CHANNELS];          /* Array Containing configuration of Output Pins       */
    IfxEgtm_Pwm_InterruptConfig interruptConfig;               /* Interrupt Configuration of Master Channel           */

    /* Default values for Single Phase Shift Modulation Strategy */
    g_egtmAtomDAB.spsRad            = INIT_PHASE_SHIFT_RAD;    /* */
    g_egtmAtomDAB.chargingDirection = IfxPrim2Sec;             /* Selected Charging Direction */
    g_egtmAtomDAB.rtState           = IfxRun;                  /* Allow runtime update        */
     ...
     */
    /* CH0 configurations */
    output[0].pin                   = (IfxEgtm_Pwm_ToutMap*)MASTER_CHANNEL;      /* Master Channel                    */
    ...
    /* CH1 configurations */
    output[1].pin                   = (IfxEgtm_Pwm_ToutMap*)HVLV_PRI_PWM_T1;     /* HVLV Primary PWM High-side (T1)   */
    output[1].complementaryPin      = (IfxEgtm_Pwm_ToutMap*)HVLV_PRI_PWM_T2;     /* HVLV Primary PWM Low-side (T1)    */
    ...
    /* CH2 configurations */
    output[2].pin                   = (IfxEgtm_Pwm_ToutMap*)HVLV_PRI_PWM_T3;     /* HVLV Secondary PWM High-side (T3) */
    output[2].complementaryPin      = (IfxEgtm_Pwm_ToutMap*)HVLV_PRI_PWM_T4;     /* HVLV Secondary PWM Low-side (T4)  */
    ...
    /* CH3 configurations */
    output[3].pin                   = (IfxEgtm_Pwm_ToutMap*)HVLV_SEC_PWM_T1;     /* HVLV Secondary PWM High-side (T1) */
    output[3].complementaryPin      = (IfxEgtm_Pwm_ToutMap*)HVLV_SEC_PWM_T2;     /* HVLV Secondary PWM Low-side (T2)  */
    ...
    /* CH4 configurations */
    output[4].pin                   = (IfxEgtm_Pwm_ToutMap*)HVLV_SEC_PWM_T3;     /* HVLV Secondary PWM High-side (T3) */
    output[4].complementaryPin      = (IfxEgtm_Pwm_ToutMap*)HVLV_SEC_PWM_T4;     /* HVLV Secondary PWM Low-side (T4)  */
    ...
    dtmConfig[2].deadTime.falling   = DEAD_TIME;              /* HVLV Secondary Falling Dead time in seconds (T3,T4) */
    ...
    interruptConfig.vmId          = IfxSrc_VmId_0;                 /* VM ID for interrupt routing                    */
    ...
    channelConfig[3].timerCh      = IfxEgtm_Pwm_SubModule_Ch_3; /* Atom channel index to be used for sync channel     */
    ...
    channelConfig[4].timerCh      = IfxEgtm_Pwm_SubModule_Ch_4; /* Atom channel index to be used for sync channel     */
    ...
    config.cluster                = IfxEgtm_Cluster_2;                  /* Cluster                                    */
    ...
    /* Initialize the EGTM */
    IfxEgtm_Pwm_init(&g_egtmAtomDAB.pwm, &g_egtmAtomDAB.channels[0], &config);

    /* 8. Store the current duty values, dead-times and phases for runtime calls */
    g_egtmAtomDAB.dutyCycles[0]= channelConfig[0].duty;
    g_egtmAtomDAB.dutyCycles[1]= channelConfig[1].duty;
    g_egtmAtomDAB.dutyCycles[2]= channelConfig[2].duty;
    g_egtmAtomDAB.dutyCycles[3]= channelConfig[3].duty;
    g_egtmAtomDAB.dutyCycles[4]= channelConfig[4].duty;
    g_egtmAtomDAB.deadTimes[0]= channelConfig[0].dtm->deadTime;
    g_egtmAtomDAB.deadTimes[1]= channelConfig[1].dtm->deadTime;
    g_egtmAtomDAB.deadTimes[2]= channelConfig[2].dtm->deadTime;
    g_egtmAtomDAB.deadTimes[3]= channelConfig[3].dtm->deadTime;
    g_egtmAtomDAB.deadTimes[4]= channelConfig[4].dtm->deadTime;
    g_egtmAtomDAB.phases[0]= channelConfig[0].phase;
    g_egtmAtomDAB.phases[1]= channelConfig[1].phase;
    g_egtmAtomDAB.phases[2]= channelConfig[2].phase;
    g_egtmAtomDAB.phases[3]= channelConfig[3].phase;
    g_egtmAtomDAB.phases[4]= channelConfig[4].phase;
}

/* This function sets the phases of the PWM channels */
void updateEgtmAtomDABPhase(float32 spsRadIn)
{
    float32 spsRad;
    float32 spsRadAbs;
    ...
    if(g_egtmAtomDAB.rtState == IfxRun)
    {
        ...
        if(spsRad >= 0)
        {
            g_egtmAtomDAB.chargingDirection = IfxPrim2Sec;
        }

        if(spsRad < 0)
        {
            g_egtmAtomDAB.chargingDirection = IfxSec2Prim;
        }

        /* Calculate phases */
        spsRadAbs = __absf(spsRad);
        if(g_egtmAtomDAB.chargingDirection == IfxPrim2Sec)         /* Charge the Secondary Bridge from Primary Bridge */
        {
            g_egtmAtomDAB.phases[1] = 0.0f;                        /* Phase of HVLV_PRI_PWM_T1, HVLV_PRI_PWM_T2       */
            g_egtmAtomDAB.phases[2] = IFX_PI;                      /* Phase of HVLV_PRI_PWM_T3, HVLV_PRI_PWM_T4       */
            g_egtmAtomDAB.phases[3] = spsRadAbs;                   /* Phase of HVLV_SEC_PWM_T1, HVLV_SEC_PWM_T2       */
            g_egtmAtomDAB.phases[4] = spsRadAbs + IFX_PI;          /* Phase of HVLV_SEC_PWM_T3, HVLV_SEC_PWM_T4       */
        }
        else                                                      /* Charge the Primary Bridge from Secondary Bridge */
        {
            g_egtmAtomDAB.phases[1] = spsRadAbs;                   /* Phase of HVLV_PRI_PWM_T1, HVLV_PRI_PWM_T2       */
            g_egtmAtomDAB.phases[2] = spsRadAbs + IFX_PI;          /* Phase of HVLV_PRI_PWM_T3, HVLV_PRI_PWM_T4       */
            g_egtmAtomDAB.phases[3] = 0.0f;                        /* Phase of HVLV_SEC_PWM_T1, HVLV_SEC_PWM_T2       */
            g_egtmAtomDAB.phases[4] = IFX_PI;                      /* Phase of HVLV_SEC_PWM_T3, HVLV_SEC_PWM_T4       */
        }
    }
    /* Update the iLLD Framework */
    IfxEgtm_Pwm_updateChannelsPhase(&g_egtmAtomDAB.pwm, (float32*)&g_egtmAtomDAB.phases[0]);
}
